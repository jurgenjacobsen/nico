...
export class Gradient {
  ...
  toString(ctx: ctx2D) {
    var grad = this.type === 'linear' ? ctx.createLinearGradient(0, 0, ctx.w, ctx.h) : ctx.createRadialGradient(ctx.w / 2, ctx.h / 2, ctx.w / 2, ctx.w / 2, ctx.h / 2, ctx.w / 2);
...
export async function DrawCard(options: CardOptions): Promise<Buffer> {
  ...
  if (options.avatar) {
    ...
    else if (typeof options.avatar === 'string' || options.avatar instanceof Buffer) ctx.drawImage(await loadImage(options.avatar), radius / 4, radius / 4, radius * 2, radius * 2);
